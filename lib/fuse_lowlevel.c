			...
			const void *op_in, const void *in_payload)
{
	...
	if (arg->valid & FATTR_DARWIN_CTIME) {
		struct fuse_setattr_in *arg = (struct fuse_setattr_in *)op_in;
	...
	if (req->se->version.darwin_extensions_enabled) {
		if (req->se->op.setattr.darwin) {
			...
			valid &= FUSE_SET_ATTR_MODE | FUSE_SET_ATTR_UID |
				 ...
				 FUSE_SET_ATTR_CTIME | FUSE_SET_ATTR_BTIME |
				 FUSE_SET_ATTR_BKUPTIME	| FUSE_SET_ATTR_FLAGS;

			req->se->op.setattr.darwin(req, nodeid, &attr, valid,
						   fi);
		} else
			fuse_reply_err(req, ENOSYS);
	} else {
		if (req->se->op.setattr.vanilla) {
			...
			req->se->op.setattr.vanilla(req, nodeid, &stbuf, valid,
						    fi);
		} else
			fuse_reply_err(req, ENOSYS);
	}
		      ...
		      const void *inarg)
{
	...
	if (req->se->conn.proto_major == 7 && req->se->conn.proto_minor == 19
	    && (req->se->conn.capable_darwin & FUSE_DARWIN_CAP_RENAME_EXT)) {
			 ...
			 const void *op_in, const void *in_payload)
{
	...
	if (req->se->version.darwin_extensions_enabled) {
		if (req->se->op.getxattr.darwin)
			req->se->op.getxattr.darwin(req, nodeid, in_payload,
		...
		if (req->se->op.getxattr.vanilla) {
			 ...
			 * zero.
			 */
			if (strcmp(in_payload, "com.apple.ResourceFork") == 0) {
				fuse_reply_err(req, ENOTSUP);
				return;
			}

			req->se->op.getxattr.vanilla(req, nodeid, in_payload,
						     arg->size);
		} else
			fuse_reply_err(req, ENOSYS);
	}
...
#ifdef __APPLE__
static void _do_monitor(fuse_req_t req, fuse_ino_t nodeid, const void *op_in,
			const void *in_payload)
{
	(void)in_payload;
	const struct fuse_monitor_in *arg = op_in;
...
{
	_do_monitor(req, nodeid, inarg, NULL);
}

static void _do_setvolname(fuse_req_t req, fuse_ino_t nodeid, const void *op_in,
			   const void *in_payload)
{
	(void)nodeid;
	(void)op_in;
...
{
	_do_setvolname(req, nodeid, NULL, inarg);
}
#endif
...
#ifdef __APPLE__
bool fuse_darwin_set_feature_flag(struct fuse_conn_info *conn,
				  uint64_t flag)
{
...
void fuse_darwin_unset_feature_flag(struct fuse_conn_info *conn,
				    uint64_t flag)
{
	conn->want_darwin &= ~flag;
}

bool fuse_darwin_get_feature_flag(struct fuse_conn_info *conn,
				  uint64_t flag)
{
	 ...
	 const void *in_payload)
{
	...
	if (arg->minor <= 19) {
		 ...
		 * TODO(bf)
		 *
		 * The macFUSE kernel extension supports buffer sizes of up to
		 * 32 MB. We pretend the kernel sent FUSE_MAX_PAGES to remove
		 * the 128 KB (Intel) / 512 KB (Apple Silicon) buffer size
		 * limit. We should clean this up when moving to 7.20.
		 */

		struct fuse_init_in *arg = (struct fuse_init_in *)op_in;
...
struct fuse_darwin_pending_mount {
 ...
 static int fuse_darwin_mount_register(struct fuse_session *se)
 {
...
static int fuse_session_mount_start(struct fuse_session *se);
			     ...
			     void *context)
{
	...
	while (true) {
		...
		(void)fuse_session_mount_start(se);
		fuse_session_put(se);
	}
	return 0;
}

static void fuse_session_mount_complete(struct fuse_session *se, int status)
{
...
out:
	...
	fuse_session_mount_complete(mc->se, completion_status);
...
static int fuse_session_mount_start(struct fuse_session *se)
{
	...
	if (se->mount_state != FUSE_DARWIN_MOUNT_DELAYED) {
	...
	if (mfch == NULL) {
		...
		pthread_cond_broadcast(&se->mount_cond);
		pthread_mutex_unlock(&se->mount_lock);
		fuse_session_mount_complete(se, -1);
		return -err;
	}
...
int fuse_session_mount(struct fuse_session *se, const char *_mountpoint)
{
	...
	if (fuse_darwin_mount_register(se) != 0) {
		pthread_mutex_unlock(&se->mount_lock);
		fuse_log(FUSE_LOG_ERR, "fuse: failed to register mount\n");
		goto error_out;
	}
...
int fuse_session_mfch(struct fuse_session *se, MFChannelRef *mfchp)
{
	...
	*mfchp = NULL;
	...
	while (true) {
		...
		case FUSE_DARWIN_MOUNT_DELAYED:
			pthread_mutex_unlock(&se->mount_lock);
			err = fuse_session_mount_start(se);
			if (err != 0 && err != EAGAIN)
				return err;
			pthread_mutex_lock(&se->mount_lock);
			break;
...
static void fuse_session_unmount_start_thread(struct fuse_session *se)
{
	pthread_attr_t attr;
	pthread_t thread;
	int err;
...
void fuse_session_unmount(struct fuse_session *se)
{
	...
	while (true) {
		...
		case FUSE_DARWIN_MOUNT_MOUNTED:
			pthread_mutex_unlock(&se->mount_lock);
			fuse_session_unmount_start_thread(se);
			return;
